...
| `paths`                     | No       | `terraform/*/*.tf`               | Only produce new versions if the PR includes changes to files that match one or more glob patterns or prefixes. Pattern syntax is in gobwas/glob [glob.Compile](https://godoc.org/github.com/gobwas/glob#Compile).                                                                         |
| `ignore_paths`              | No       | `.ci/`                           | Inverse of the above. Pattern syntax is documented in the library gobwas/glob [glob.Compile](https://godoc.org/github.com/gobwas/glob#Compile), or a path prefix can be specified (e.g. `.ci/` will match everything in the `.ci` directory).                                              |
